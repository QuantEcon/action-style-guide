...
- **Bumped GitHub Actions to Node 24-compatible versions** — GitHub forces Node 24 as the default runner runtime from 2026-06-02 (Node 20 fully removed 2026-09-16). Updated `actions/checkout@v4→v5` and `astral-sh/setup-uv@v3→v7` in `action.yml` and CI; the docs workflow now uses `actions/setup-node@v4→v7` with `node-version: '24'` for the `mystmd` build, `actions/upload-pages-artifact@v3→v5`, and `actions/deploy-pages@v4→v5`. Resolves #16.
- **Bumped example workflows to `@v0.8`** — `examples/style-guide-comment.yml` and `examples/style-guide-weekly.yml` pinned the action at the long-stale `@v0.3`; they now track the `v0.8` release line, matching the `docs/user/*` snippets.
- **Moved the example workflows and usage docs onto Node 24** — `examples/style-guide-comment.yml` and `examples/style-guide-weekly.yml` now pin `actions/github-script@v7→v9` (`runs.using: node24`; the example scripts use only `github.rest.*` and `context`, which v9's breaking changes do not touch). The `docs/user/*` snippets move from `@v0.7→@v0.8`, because the `v0.7` tag's `action.yml` still nests Node 20 actions (`actions/checkout@v4`, `actions/setup-python@v5`), while `main`'s nests only Node 24 ones. `docs/user/github-app-setup.md` also moves `actions/create-github-app-token@v1→v3` (the first Node 24 major). A self-hosted runner needs v2.327.1 or later to run these Node 24 actions, and `create-github-app-token` v3 no longer handles proxies itself: behind `HTTP_PROXY` or `HTTPS_PROXY`, also set `NODE_USE_ENV_PROXY=1` on that step. Part of QuantEcon/workspace-lectures#68.
